...
def train(g_par, d_par, gan_model, dataset_real, u_sampled_data,  n_epochs, n_batch, n_critic, clip_val, n_patch, f):
    ...
    for i in range(n_epochs):
        ...
        for j in range(bat_per_epo):
            ...
            g_loss = gan_model.train_on_batch ([X_gen_inp], [y_gan, X_r, X_r, X_r])
            
            f.write('>%d, %d/%d, d=%.3f, acc = %.3f,  w=%.3f,  mae=%.3f,  mssim=%.3f, wvt=%.7f, g=%.3f' %(i+1, j+1, bat_per_epo, d_loss, accuracy, g_loss[1], g_loss[2], g_loss[3], g_loss[4], g_loss[0]))
            f.write('\n')
            print ('>%d, %d/%d, d=%.3f, acc = %.3f,  w=%.3f,  mae=%.3f,  mssim=%.3f, wvt=%.7f, g=%.3f' %(i+1, j+1, bat_per_epo, d_loss, accuracy, g_loss[1], g_loss[2], g_loss[3], g_loss[4], g_loss[0]))
        ...
        if (i+1)%5==0:
          filename = '/home/Co-VeGAN/covegan_a5_disc_%04d.h5' % (i+1)
...
#g_par.layers[-2].set_weights(g_model.get_weights())

gan_model = define_gan_model(g_par, d_par, in_shape_gen)
opt1 = Adam(lr = 0.0002, decay = 0.0015, beta_1 = 0.5)
gan_model.compile(loss = [wloss, 'mae', mssim, wt_diff], optimizer = opt1, loss_weights = [0.01, 20.0, 1.0, 10.0]) #loss weights for generator training
'''
